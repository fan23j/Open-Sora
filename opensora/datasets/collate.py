...
def calculate_padding(tensor: torch.Tensor, max_shape: List[int]) -> List[int]:
    """Calculate the padding for each dimension to match the max_shape."""
    padding = []
    for current_size, max_size in zip(tensor.size()[::-1], max_shape[::-1]):
        pad_amount = max_size - current_size
        padding.extend([0, pad_amount])  # Pad at the beginning and end equally
    return padding
