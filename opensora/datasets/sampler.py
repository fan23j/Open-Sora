...
class VariableNBAClipsBatchSampler(DistributedSampler):
    def __init__(
        ...
        self.dataset: VariableNBAClipsDataset = dataset
    ...
    def group_by_bucket(self) -> dict[Tuple[str, int, str], List[str]]:
        ...
        assert type(self.dataset) is VariableNBAClipsDataset, f"Error: dataset.ann is {type(self.dataset.ann)}"
        bucket_sample_dict = {('360p', 64, '1.00'): self.dataset.filtered_ds.filtered_clip_annotations_file_paths[:100]}
