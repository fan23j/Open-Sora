...
class VariableVideoTextDataset(VideoTextDataset):
    ...
    def getitem(self, index):
        ...
        if file_type == "video":
            # loading
            #TODO: @levi add to data path. fix the inconsistent paths. ok, some videos are just missing?
            video_name = video_path.split('/')[-1]
            name_parts = video_name.split('_')
            parent1, parent2 = name_parts[0], name_parts[1]
            path = f'/mnt/mir/fan23j/data/nba-plus-statvu-dataset/filtered-clips/{parent1}/{parent2}/{video_name}'
