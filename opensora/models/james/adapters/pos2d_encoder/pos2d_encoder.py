...
class Pos2D_Encoder(nn.Module):
    def __init__(self, num_frames=64, num_instances=11, d_model=1152, nhead=4, num_layers=2, device='cuda', dtype=torch.bfloat16):
        super().__init__()
        self.d_model = d_model
        self.device = device
        self.dtype = dtype
        
        # Embedding layer to project 2D positions to d_model dimensions (set this to 3 cuz ball has a z axis value)
        self.embed = nn.Linear(3, d_model, dtype=dtype)
    ...
    def forward(self, x):
        ...
        # x shape: [batch_size, num_instances, num_frames, 3]
