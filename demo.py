...
serve.start(http_options={"host": os.environ["HOST"], "port": int(os.environ["PORT"])})
...
class Args:
    def __init__(self, model_type="v1.1-stage3", model=None, output="./outputs", share=False, enable_optimization=False):
...
app = GradioServer.options(num_replicas=int(os.environ["NUM_REPLICA"]), ray_actor_options={"num_gpus": 1, "num_cpus": 8}).bind(
    gradio_builder
)